...
```{r}
chisq.test(c(5065,7779), p=c(0.492, 0.508))
```
*test showed that p is less than 0.5, we reject null hypothesis. It means that heart attacks does not happen in this proportion*
...
```
*p-value is smaller than 0.5, reject null hypothesis, survival is not dependent on gender*
...
```{r}
table(heart_attacks$AGE)
```
```{r}
heart_attacks$AGE<-as.numeric(as.character(heart_attacks$AGE))
```

```{r}
colnames(heart_attacks)
```
```{r}
lessthan30<-subset(heart_attacks, AGE<30)
```
```{r}
View(lessthan30)
```
...
```{r}
smalltable<-table(lessthan30$SEX, lessthan30$DIED)
smalltable
```
```{r}
chisq.test(smalltable)
```
*chi squared test gave p-value greater than 0.5 so we fail to reject hypothesis, so survival does not depend on sex or gender*
...
```{r}
table(table(survey$Smoke, survey$Exer))
```
```{r}
table
```
```{r}
chisq.test(table(table(survey$Smoke, survey$Exer)))
```
*chi test showed that p-value is greater than 0.5 meaning that smoking and exercising not dependent*
